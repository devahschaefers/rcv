...
from rcv.commands.build import build_latex, build_typst
...
class ResumeWatcher(FileSystemEventHandler):
    ...
    def __init__(self, resume_file: Path, output_dir: Path, format: str, compiler: str):
    ...
    def on_modified(self, event):
        ...
        if self.format == "latex":
            success = build_latex(self.resume_file, self.output_dir, self.compiler)
        else:
            success = build_typst(self.resume_file, self.output_dir, self.compiler)
...
def watch(
    ...
    if resume.metadata.format == "latex":
        success = build_latex(resume_file, output_dir, compiler)
    else:
        success = build_typst(resume_file, output_dir, compiler)
